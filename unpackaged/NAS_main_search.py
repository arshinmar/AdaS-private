...
if __name__ == '__main__':
    ...
    '''
    output_sizes = [96,33,18,24,18]
    output_path_string = str(output_path) +'\\'+'full_conv=96x5_thresh='+str(GLOBALS.CONFIG['adapt_rank_threshold'])+'_beta='+str(GLOBALS.CONFIG['beta'])
    output_path_full = output_path / f"full_conv=96x5_thresh={GLOBALS.CONFIG['adapt_rank_threshold']}_beta={GLOBALS.CONFIG['beta']}"
    ...
    #'''
    #new_model_state_dict = prototype(GLOBALS.NET.state_dict(),output_sizes)
    new_model_state_dict = prototype(torch.load('model_weights'+'\\'+'model_state_dict_96,96,96,96,96_thresh=0.3'),output_sizes)
    new_network=AdaptiveNet(num_classes=10, new_output_sizes=output_sizes)
    new_network.load_state_dict(new_model_state_dict)
    #'''
    ...
    output_path_string = str(output_path) +'\\'+'fresh_conv=96x5_thresh='+str(GLOBALS.CONFIG['adapt_rank_threshold'])+'_beta='+str(GLOBALS.CONFIG['beta'])
    output_path_fresh = output_path / f"fresh_conv=96x5_thresh={GLOBALS.CONFIG['adapt_rank_threshold']}_beta={GLOBALS.CONFIG['beta']}"
